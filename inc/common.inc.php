...
function get_mostviewed_articles() {
    global $cid;
    $sql = "SELECT DISTINCT article,COUNT(article) AS c FROM (SELECT article FROM article_visit AS av INNER JOIN article AS a ON (av.article=a.id) WHERE a.published IS NOT NULL AND av.timestamp > (UNIX_TIMESTAMP() - 1814400) ORDER BY timestamp DESC LIMIT 500) AS t GROUP BY article ORDER BY c DESC LIMIT 5";
    $rsc = mysql_query($sql,$cid);
    while ($row = mysql_fetch_array($rsc))
